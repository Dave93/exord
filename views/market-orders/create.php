...
foreach ($folders as $folder) {
    $itemList = "";
    $items = Products::getProducts($folder['id'], $model->id, Yii::$app->user->id, true);
    foreach ($items as $item) {
        ...
        if (in_array($item['id'], $availability)) {
            $availableInput = <<<HTML
 <input type="text" class="form-control availability" name="Available[{$item['id']}]" value="" />
HTML;
        }
        ...
        <tr id="p-{$item['id']}">
            ...
            <td width="200">
                <input type="number" class="form-control quantity" name="Items[{$item['id']}]" value="{$item['quantity']}" step="any" />
            </td>
            <!-- td width="200">
                {$availableInput}
            </td -->
