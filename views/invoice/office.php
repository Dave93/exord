...
<div class="card">
    ...
    <div class="content">
        ...
        $form = ActiveForm::begin([
            'id' => 'filterForm',
            'method' => 'get',
            'action' => ['/invoice/office']
        ]);
        ?>
        <div class="container-fluid">
            <div class="row">
                <div class="input-daterange datepicker align-items-center" data-date-format="dd.mm.yyyy"  data-date-start-date="-10y" data-date-end-date="+10y"
                     data-today-highlight="1" >
                    <div class="col-xs-2">
                        <div class="form-group">
                            <input name="start" class="form-control" placeholder="Дата с" type="text"
                                   autocomplete="off"  data-date-start-date="-10y" data-date-end-date="+10y"
                                   value="<?= $start ?>"/>
                        </div>
                    </div>
                    <div class="col-xs-2">
        ...
        <?= GridView::widget([
            ...
            'rowOptions' => function ($model) use ($arIncorrectOrders) {
                $class = "";
                if ($model->state == 1)
                    $class = "bg-orange";
                ...
                if (in_array($model->id, $arIncorrectOrders)) {
                    $class .= " bg-danger";
                }
                return ['class' => $class];
            },
            'columns' => [
                ...
                [
                    ...
                    'value' => function ($model) use ($isOrderMan) {
                        ...
                        return Html::a($model->id, ['office-view', 'id' => $model->id]);
                    }
                ],
                [
                    'label' => 'Кол. позиц.',
                ...
                [
                    ...
                    'buttons'=>[
                        'view'=>function ($url, $model) {
                            $customurl=Yii::$app->getUrlManager()->createUrl(['invoice/office-view','id'=>$model['id']]);
                            return \yii\helpers\Html::a( '<span class="glyphicon glyphicon-eye-open"></span>', $customurl,
                                ['title' => Yii::t('yii', 'View'), 'data-pjax' => '0']);
                        },
                        'approve' => function($url, $model) use ($arIncorrectOrders) {
                            if (in_array($model->id, $arIncorrectOrders)) {
                                return "";
                            }
                            $customurl=Yii::$app->getUrlManager()->createUrl(['invoice/approve','id'=>$model['id'], 'back' => Yii::$app->request->url]);
