...
// Собираем уже выбранные продукты
$selectedProducts = [];
...
$selectedProductsJson = Json::encode($selectedProducts);
...
var selectedProductsData = $selectedProductsJson;

// Загружаем уже выбранные продукты
selectedProductsData.forEach(function(product) {
    addProductRow(product.id, product.name, product.unit, product.count);
});
...
function addProductRow(productId, productName, productUnit, count) {
    ...
    html += '<td width="200"><input type="number" class="form-control quantity-input" name="items[' + productId + ']" value="' + (count || '') + '" step="any" min="0.01" /></td>';
    ...
    // Автофокус на поле количества если это новый продукт
    if (!count) {
        $('#row-' + productId + ' .quantity-input').focus();
    }
}
...
<div class="card">
    ...
    <div class="content">
        ...
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="comment" rows="3" placeholder="Добавьте комментарий к списанию..." required><?= Html::encode($model->comment) ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="form-group">
                    <label>Прикрепленные фотографии</label>
                    <div class="existing-photos">
                        <?php foreach ($model->photos as $photo): ?>
                            <div class="photo-item">
                                <a href="<?= $photo->getFileUrl() ?>" target="_blank">
                                    <img src="<?= $photo->getFileUrl() ?>" alt="Фото">
                                </a>
                                <?= Html::a(
