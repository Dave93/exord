...
/**
 * MarketOrdersController implements the CRUD actions for Orders model.
 */
...
//                        continue;
                ...
                $bot = new TelegramBot();
                $message = $bot->sendDocument(-1002270767081, 'https://les.iiko.uz/'.$filePath);
                $bot->sendMessage(-1002270767081, $text, 'HTML', false, $message['result']['message_id']);
                return $this->redirect(['market-orders/view', 'id' => $model->id]);
            }
        }
...
//                $oi->supplierQuantity = 0;
    ...
    public function actionClose($id)
    {
        ...
        Yii::info("=== MarketOrders actionClose: Начало закрытия заказа #{$id} ===", 'iiko');
        ...
        Yii::info("=== MarketOrders actionClose: Завершение обработки заказа #{$id} ===", 'iiko');
        return $this->redirect(['orders/view', 'id' => $model->id]);
    }
...
//        $searchModel->state = 0;
    ...
    public function actionStock($tab = null)
    {
        ...
        $orders = $query->select("stores.id,stores.name,orders.id,orders.date,orders.storeId,orders.comment,orders.addDate,orders.state")
            ->from("orders")
            ->leftJoin("stores", "stores.id=orders.storeId")
            ->where("orders.state<1 and storeId!=:s and is_market=1", [":s" => $stockId])
            ->orderBy("stores.name")
            ->all();
...
//                echo '<pre>'; print_r($outDoc); echo '</pre>';die();
            ...
            if ($noRedirect) {
                ...
                return $this->redirect(['market-orders/stock', 'tab' => $orderId]);
            }
        }
